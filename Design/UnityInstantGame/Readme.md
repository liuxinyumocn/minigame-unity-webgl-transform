...
| 纹理压缩格式   | DXT、ASTC、ETC | ASTC                            |
| 自建CDN        | 支持           | 不支持(使用腾讯云CDN服务)       |
...
​		Unity 转换至微信小游戏平台运行目前有两种方案，即 [Unity WebGL小游戏快适配](https://github.com/wechat-miniprogram/minigame-unity-webgl-transform/blob/main/Design/Summary.md)  与 本文所介绍的 Instant Game 提供的 Auto Steaming 加载方案。Instant Game 对于极小的休闲类游戏将具备更快更小修改代价将游戏迁移至微信小游戏环境内运行，若追求较好的游戏品质，无论哪一种方案都需要开发者专项的适配。从运行性能而言，两者差别不是很大，对于 微信快适配 工具而言，提供了特有异步常见纹理的压缩工具、wasm代码分包等能力，对于 Instant Game 能够对多种游戏资源均支持异步加载的方式，从网络总量而言 Instant Game 的请求次数更多。从部署角度而言 Instant Game 需要使用平台提供的 CDN 服务，不支持开发者自定义。
